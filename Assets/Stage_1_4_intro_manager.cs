...
public class Stage_1_4_intro_manager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(!dialogue[0].isFinish&&dialogueManager.state==DialogueState.End){
            ...
            dialogueManager.StartDialogue(dialogue[1]);
            dialogue[1].isFinish=true;
        }if(!boxAnimator.GetBool("isActive")&&dialogue[1].isFinish&&dialogueManager.state==DialogueState.End&&!rewardScreenOn){
            boxAnimator.SetBool("isActive",true);
            rewardScreenOn=true;

        }if(!boxAnimator.GetBool("isActive")&&!dialogue[2].isFinish&&dialogueManager.state==DialogueState.End){
            dialogueManager.StartDialogue(dialogue[2]);
            dialogue[2].isFinish=true;
            
        }if(dialogue[2].isFinish&&dialogueManager.state==DialogueState.End){
            boxAnimator.SetBool("isActive",false);
        }
        if(dialogue[2].isFinish&&dialogueManager.state==DialogueState.End&&!boxAnimator.GetBool("isActive")){
